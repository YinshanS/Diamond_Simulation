...
b=bar3(dU1);
title({'Welfare Effect of Constant Debt Policy with Tax and Subsidy','(Log Utility, 2-state Uncertainty)','(\alpha=0.35, \beta=0.495, T=200, A=10\beta^{1-\alpha}/3)'});
ylabel('Standard Deviation of 2-state Shock','Position',[0.148031275483167,2.386260878796423,-0.000757954325086]); 
xlabel('Debt Amount') ;
xticklabels({'0.0001','0.0002','0.0003','0.0004','0.0005','0.0006','0.0007','0.0008','0.0009','0.001'});
yticklabels({'0.4','0.45','0.5','0.55','0.6','0.65','0.7','0.75','0.8'});
zlabel('Welfare Change');
savefig('Welfare Effect Log U Constant Debt.fig');
...
Y(1) = A(1)*K0^alpha;
W(1) = (1-alpha)*Y(1);
K(1) = beta*W(1);%invest beta or all wage(if wage<beta)
cy(1) = W(1)-K(1);
co(1) = Y(1)-W(1);
...
Q1(1) = A(1)*K0^alpha;
W1(1) = (1-alpha)*Q1(1);
K1(1) = W1(1)*beta-D;%invest beta or all wage(if wage<beta)
cy1(1) = W1(1)-K1(1)-D;
co1(1) = Q1(1)-W1(1)+D;
r1(1) = alpha*A(1)*K0^(alpha-1);
tau(1) = 1-r(1)/r1(1);
...
for t=2:T
    ...
    cy(t) = (1-beta)*W(t);
    co(t) = alpha*Y(t);
    U(t-1) = (1-beta)*log(cy(t-1))+beta*log(co(t))+P(t);
    r(t) = alpha*Y(t)/K(t-1);  
    
    Q1(t) = A(t)*K1(t-1)^alpha;
    r1(t) = alpha*Q1(t)/K1(t-1);
    tau(t) = 1-r(t)/r1(t);
    rsb1(t) = (1-tau(t))*(2*alpha*A0*(beta*W1(t-1)-D1(t-1))^alpha*(1-sigma^2))/(beta*W1(t-1)-2*D1(t-1)+sqrt(4*sigma^2*(D1(t-1)^2-beta*W1(t-1)*D)+beta^2*W1(t-1)^2));
    D1(t)=max(D, D1(t-1)*rsb1(t)+(W(t-1)-W1(t-1))*r(t));
    %D1(t)=max(D, D1(t-1)*rsb1(t));%constant debt
    %D1(t)=D1(t-1)*rsb1(t)+W(t-1)-W1(t-1);
    %D1(t)=D1(t-1)*rsb1(t)+(W(t-1)-W1(t-1))*r(t);
    W1(t) = Q1(t)-r(t)*K1(t-1);
    %W1(t) = (1-alpha)*Y1(t);
    K1(t) = max(beta*W1(t)-D1(t),0);
    ...
    U1(t-1) = (1-beta)*log(cy1(t-1))+beta*log(co1(t))+P1(t);
end
dU = mean(U1-U);
end
%{
